...
```
Visually checking, the data looks very normal after 1000 resamples. Now onto the t-test!
...
```{r bootstrap}
data.frame(boot = 1:1000) %>% # how many repetitions are needed?
    group_by(boot) %>%
...
This shows that the means are very different from each other based on the daily dose amount. A conclusion we can draw from this, that based on all data (irrespective of the method) higher doses of Vitamin C result in faster tooth growth.
